...
export default function CustomerInquiriesPage() {
  ...
  const fetchTickets = useCallback(async () => {
    setLoading(true);
    try {
      // Filter by current user's email to show only their inquiries
      const userEmail = session?.user?.email || "";
      const res = await fetch(
        `/api/admin/support?search=${encodeURIComponent(search)}&status=${statusFilter}&isInternal=false&contactEmail=${encodeURIComponent(userEmail)}`
      );
  ...
  }, [session?.user?.email, search, statusFilter]);
  ...
  useEffect(() => {
    if (status === "authenticated" && tickets.length >= 0) {
      const hasShownWelcome = sessionStorage.getItem("welcome-notification-shown");
      if (!hasShownWelcome) {
        setTimeout(() => {
          showToast("👋 Welcome to Customer Inquiries! Select a ticket to view details and respond.");
        }, 1000);
  ...
  useEffect(() => {
    ...
    const initKnown = async () => {
      try {
        const userEmail = session?.user?.email || "";
        const res = await fetch(`/api/admin/support?isInternal=false&contactEmail=${encodeURIComponent(userEmail)}`);
        const data = await res.json();
    ...
    const interval = setInterval(async () => {
      try {
        const userEmail = session?.user?.email || "";
        const res = await fetch(`/api/admin/support?isInternal=false&status=all&contactEmail=${encodeURIComponent(userEmail)}`);
        const data = await res.json();
        if (data.ok && data.tickets) {
  ...
  const handleCreateInquiry = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.ok) {
        showToast("Inquiry submitted successfully! Check your email for confirmation.");
  ...
  const handleReplySubmit = async (e: React.FormEvent, id: string) => {
    ...
    try {
      const res = await fetch(`/api/admin/support/${id}/reply`, {
        ...
        body: JSON.stringify({ 
          message: replyText,
          sender: "admin"  // Explicitly set sender as admin
        }),
      });
      const data = await res.json();
      if (data.ok) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/admin/support/${deleteConfirmId}`, { method: "DELETE" });
      const data = await res.json();
      if (data.ok) {
        showToast("Ticket deleted successfully");
        setTickets((prev) => prev.filter((t) => t._id !== deleteConfirmId));
        if (selectedTicket?._id === deleteConfirmId) setSelectedTicket(null);
      } else {
                ...
                {isAdmin && (
                  <div className="border-t border-slate-100 p-4 bg-white">
                    ...
                    <p className="text-[10px] text-slate-400 mt-2 flex items-center gap-1.5">
                      ...
                      Reply will be sent to visitor via email.
                    </p>
                  </div>
                )}
                    ...
                    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                      ...
                      <p className="text-xs text-slate-600 leading-relaxed whitespace-pre-wrap">
                        {selectedTicket.chatContext && selectedTicket.chatContext.length > 0 
                          ? selectedTicket.chatContext[0].content 
                  ...
                  {selectedTicket.replies && selectedTicket.replies.length > 0 && (
                    <div className="space-y-2">
                      ...
                      <div className="space-y-3">
                        {selectedTicket.replies.filter(r => r.sender === "admin").map((r, i) => (
                          <div
                            key={i}
                            className="flex justify-end"
                          >
                            <div className="px-4 py-2.5 rounded-2xl text-xs leading-relaxed max-w-[80%] shadow-sm bg-slate-800 text-white rounded-tr-none">
            ...
            >
              ...
              <div className="p-5 border-b border-slate-100 bg-slate-50/50">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-sm font-bold text-slate-900">Delete Ticket</h3>
              ...
              <div className="p-5">
                <p className="text-xs text-slate-600 leading-relaxed">
                  Are you sure you want to delete this ticket? All associated messages and conversation history will be permanently removed.
                </p>
              </div>
              ...
              <div className="p-5 pt-0 flex items-center justify-end gap-2">
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-xs font-semibold text-white bg-rose-600 hover:bg-rose-700 disabled:opacity-40 transition-all active:scale-95 shadow-md shadow-rose-500/10"
                >
